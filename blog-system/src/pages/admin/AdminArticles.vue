    ...
    <div v-else class="card" style="padding: 0; overflow: hidden">
      <div class="table-wrapper">
        <table>
          ...
          <tbody>
            <tr v-for="article in articleStore.articles" :key="article._id || article.id">
              ...
              <td>
                <div class="actions-cell">
                  <!-- 修复：使用非空断言 article.id! 因为已存在的文章一定有 id -->
                  <button
                    class="btn btn-sm btn-outline"
                    @click="$router.push(`/admin/articles/${article._id || article.id}/edit`)"
                  >
                    ✏️ 编辑
                  </button>
                  <button
                    class="btn btn-sm"
                    :class="deleteConfirmId === (article._id || article.id) ? 'btn-delete-confirm' : 'btn-delete'"
                    @click="handleDelete((article._id || article.id)!)"
                  >
                    {{ deleteConfirmId === (article._id || article.id) ? '⚠️ 确认删除' : '🗑 删除' }}
                  </button>
                </div>
              </td>
            </tr>
